 ...
 */
class Hycube_GeoIP_Adminhtml_Mageworx_Geoip_DatabaseController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Start update action
     */
    ...
    public function runUpdateAction()
    {
        ...
        switch ($action) {
            ...
            case 'download':
                $errors = Mage::getModel('hycube_geoip/geoip')->downloadFile($helper->getDbUpdateSource(), $helper->getTempUpdateFile());
                if (count($errors)) {
                    $success = false;
                }
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('system/config/hycube_geoip/geoip');
    }
}
