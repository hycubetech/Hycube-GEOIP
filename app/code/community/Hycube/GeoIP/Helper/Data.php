 ...
 */
class Hycube_GeoIP_Helper_Data extends Mage_Core_Helper_Abstract
{
    const XML_GEOIP_DATABASE_TYPE = 'hycube_geoip/geoip/db_type';
    const XML_GEOIP_UPDATE_DB = 'hycube_geoip/geoip/update_db';
    ...
    public function isCityDbType()
    {
        ...
        return (Mage::getStoreConfig(self::XML_GEOIP_DATABASE_TYPE) == Hycube_GeoIP_Model_Source_Dbtype::GEOIP_CITY_DATABASE);
    }
    ...
    public function getFlagPath($name = null)
    {
        ...
        $filePath = Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . 'hycube' . DS . 'geoip' . DS . 'flags' . DS . $flagName;
        if (file_exists($filePath)) {
            $fileUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'hycube/geoip/flags/' . $flagName;
        } else {
            $fileUrl = '';
        }
