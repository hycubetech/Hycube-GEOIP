...
include_once 'geoIpRecord.php';
include_once 'geoIpDnsRecord.php';
...
class GeoIP
{
    ...
    public $geoipCountryCodeToNumber = array(
    ...
    public $geoipCountryCodes = array(
    ...
    public $geoipCountryCodes3 = array(
    ...
    public $geoipCountryNames = array(
    ...
    public static $geoIpRegionName = array(
        "AD" => array(
            "02" => "Canillo",
            "03" => "Encamp",
            "04" => "La Massana",
    ...
    {
        $this->flags = $flags;
        if ($this->flags & static::GEOIP_SHARED_MEMORY) {
            $this->shmid = @shmop_open(static::GEOIP_SHM_KEY, "a", 0, 0);
        } else {
            $this->filehandle = fopen($filename, "rb") or die("Can not open $filename\n");
            if ($this->flags & static::GEOIP_MEMORY_CACHE) {
    ...
    protected function _setupSegments()
    {
        $this->databaseType = static::GEOIP_COUNTRY_EDITION;
        $this->recordLength = static::STANDARD_RECORD_LENGTH;
        if ($this->flags & static::GEOIP_SHARED_MEMORY) {
            $offset = @shmop_size($this->shmid) - 3;
            for ($i = 0; $i < static::STRUCTURE_INFO_MAX_SIZE; $i++) {
                ...
                if ($delim == (chr(255) . chr(255) . chr(255))) {
                    ...
                    if ($this->databaseType == static::GEOIP_REGION_EDITION_REV0) {
                        $this->databaseSegments = static::GEOIP_STATE_BEGIN_REV0;
                    } else if ($this->databaseType == static::GEOIP_REGION_EDITION_REV1) {
                        $this->databaseSegments = static::GEOIP_STATE_BEGIN_REV1;
                    } else if (($this->databaseType == static::GEOIP_CITY_EDITION_REV0) ||
                        ($this->databaseType == static::GEOIP_CITY_EDITION_REV1)
                        || ($this->databaseType == static::GEOIP_ORG_EDITION)
                        || ($this->databaseType == static::GEOIP_ISP_EDITION)
                        || ($this->databaseType == static::GEOIP_ASNUM_EDITION)
                    ) {
                        $this->databaseSegments = 0;
                        $buf = @shmop_read($this->shmid, $offset, static::SEGMENT_RECORD_LENGTH);
                        for ($j = 0; $j < static::SEGMENT_RECORD_LENGTH; $j++) {
                            $this->databaseSegments += (ord($buf[$j]) << ($j * 8));
                        }
                        if (($this->databaseType == static::GEOIP_ORG_EDITION) ||
                            ($this->databaseType == static::GEOIP_ISP_EDITION)
                        ) {
                            $this->recordLength = static::ORG_RECORD_LENGTH;
                        }
                    }
                    break;
                } else {
            ...
            if (($this->databaseType == static::GEOIP_COUNTRY_EDITION) ||
                ($this->databaseType == static::GEOIP_PROXY_EDITION) ||
                ($this->databaseType == static::GEOIP_NETSPEED_EDITION)
            ) {
                $this->databaseSegments = static::GEOIP_COUNTRY_BEGIN;
            }
        } else {
            ...
            for ($i = 0; $i < static::STRUCTURE_INFO_MAX_SIZE; $i++) {
                ...
                if ($delim == (chr(255) . chr(255) . chr(255))) {
                    ...
                    if ($this->databaseType == static::GEOIP_REGION_EDITION_REV0) {
                        ...
                        $this->databaseSegments = 0;
                        $buf = fread($this->filehandle, static::SEGMENT_RECORD_LENGTH);
                        for ($j = 0; $j < static::SEGMENT_RECORD_LENGTH; $j++) {
                            $this->databaseSegments += (ord($buf[$j]) << ($j * 8));
                        }
                        if ($this->databaseType == static::GEOIP_ORG_EDITION ||
    ...
    /**
     * Replacement of the geoip_record_by_addr | GeoIP_record_by_addr
     *
     * @param $address
    ...
    public function getRecordByAddress($address)
    {
        ...
        $ipNum = ip2long($address);

        return $this->_getRecord($ipNum);
    }

    /**
     * Replacement for the geoip_country_code_by_addr
     *
     * @param $address
     * @return bool|string
     */
    public function getCountryCodeByAddress($address)
    {
        if ($this->databaseType == static::GEOIP_CITY_EDITION_REV1) {
            $record = $this->getRecordByAddress($address);
            if ($record !== false) {
                return $record->countryCode;
            }
        } else {
            $countryId = $this->getCountryIdByAddress($address);
            if ($countryId !== false) {
                return $this->geoipCountryCodes[$countryId];
            }
        }
    ...
    /**
     * Replacement for the geoip_country_name_by_addr
     *
     * @param $address
     * @return bool|string
     */
    public function getCountryNameByAddress($address)
    {
        if ($this->databaseType == static::GEOIP_CITY_EDITION_REV1) {
            $record = $this->getRecordByAddress($address);
            ...
            $countryId = $this->getCountryIdByAddress($address);
            if ($countryId !== false) {
                return $this->geoipCountryNames[$countryId];
            }
        }
    ...
    /**
     * Replacement for the geoip_country_id_by_addr
     *
     * @param $address
     * @return bool|string
     */
    public function getCountryIdByAddress($address)
    {
        $ipNum = ip2long($address);

        return $this->_geoipSeekCountry($ipNum) - static::GEOIP_COUNTRY_BEGIN;
    }

    /**
     * Replacement for the geoip_country_id_by_name
     *
     * @param $name
     * @return bool|string
     */
    public function getCountryIdByName($name)
    {
        ...
        return $this->getCountryIdByAddress($address);
    }

    /**
     * Replacement for the geoip_country_code_by_name
     *
     * @param $name
     * @return bool|string
     */
    ...
    {
        $countryId = $this->getCountryIdByName($name);
        if ($countryId !== false) {
            return $this->geoipCountryCodes[$countryId];
        }
    ...
    /**
     * Replacement for the geoip_country_name_by_name
     *
     * @param $name
     * @return bool|string
     */
    public function getCountryNameByName($name)
    {
        $countryId = $this->getCountryIdByName($name);
        if ($countryId !== false) {
            return $this->geoipCountryNames[$countryId];
    ...
    /**
     * Replacement for the geoip_region_by_addr
     *
     * @param $address
     * @return array|int
     */
    ...
    /**
     * Replacement for the geoip_org_by_addr
     *
     * @param $address
     * @return int|null|string
     */
    public function getOrgByAddress($address)
    {
        if ($address == null) {
            return 0;
        }
    ...
    /**
     * Replacement of the getrecordwithdnsservice
     *
     * @param $str
    ...
    public function getRecordWithDnsService($str)
    {
        ...
        $number = $this->geoipCountryCodeToNumber[$record->countryCode];
        $record->countryCode3 = $this->geoipCountryCodes3[$number];
        $record->countryName = $this->geoipCountryNames[$number];

        return $record;
    }

    /**
     * Replacement for the geoip_load_shared_mem
     *
     * @param $file
     */
    public function loadSharedMem($file)
    {
        $fp = fopen($file, "rb");
        if (!$fp) {
        ...
        $sArray = fstat($fp);
        $size = $sArray['size'];
        if ($shmid = @shmop_open(static::GEOIP_SHM_KEY, "w", 0, 0)) {
            shmop_delete($shmid);
            shmop_close($shmid);
        }
        $shmid = shmop_open(static::GEOIP_SHM_KEY, "c", 0644, $size);
        shmop_write($shmid, fread($fp, $size), 0);
        shmop_close($shmid);
    }

    /**
     * replacement for the getdnsattributes
     *
     * @param $l
     * @param $ip
     * @return string
    ...
    public function getDnsAttributes($l, $ip)
    {
        if (!class_exists('Net_DNS_Resolver')) {
            throw new \Exception('Net_DNS_Resolver class requires the Net_DNS package');
        }
    ...
    /**
     * Replacement for the _get_record
     *
     * @param $ipNum
     * @return geoIpRecord
     */
    protected function _getRecord($ipNum)
    {
        $seekCountry = $this->_geoipSeekCountry($ipNum);
        ...
        if ($this->flags & static::GEOIP_MEMORY_CACHE) {
            $recordBuf = substr($this->memoryBuffer, $recordPointer, static::FULL_RECORD_LENGTH);
        } elseif ($this->flags & static::GEOIP_SHARED_MEMORY) {
            $recordBuf = @shmop_read($this->shmid, $recordPointer, static::FULL_RECORD_LENGTH);
        } else {
            fseek($this->filehandle, $recordPointer, SEEK_SET);
            $recordBuf = fread($this->filehandle, static::FULL_RECORD_LENGTH);
        }
        $record = new geoIpRecord;
        $recordBufPos = 0;
        $char = ord(substr($recordBuf, $recordBufPos, 1));
        $record->countryCode = $this->geoipCountryCodes[$char];
        $record->countryCode3 = $this->geoipCountryCodes3[$char];
        $record->countryName = $this->geoipCountryNames[$char];
        $recordBufPos++;
        $strLength = 0;
        // Get region
        ...
        if ($strLength > 0) {
            $record->region = substr($recordBuf, $recordBufPos, $strLength);
        }
        ...
        if ($strLength > 0) {
            $record->city = substr($recordBuf, $recordBufPos, $strLength);
        }
        $recordBufPos += $strLength + 1;
        $strLength = 0;
        // Get postal code
        ...
        if ($strLength > 0) {
            $record->postalCode = substr($recordBuf, $recordBufPos, $strLength);
        }
        $recordBufPos += $strLength + 1;
        $strLength = 0;
        ...
        if (static::GEOIP_CITY_EDITION_REV1 == $this->databaseType) {
            $metroAreaCombo = 0;
            if ($record->countryCode == "US") {
                ...
                $record->metroCode = $record->dmaCode = floor($metroAreaCombo / 1000);
    ...
    /**
     * Replacement for the _geoip_seek_country
     *
     * @param $ipNum
     * @return bool|mixed
     */
    protected function _geoipSeekCountry($ipNum)
    {
        $offset = 0;
    ...
    /**
     * Replacement for the _get_org
     *
     * @param $ipNum
     * @return null|string
     */
    protected function _getOrg($ipNum)
    {
        ...
        if ($this->flags & static::GEOIP_SHARED_MEMORY) {
            $orgBuf = @shmop_read($this->shmid, $recordPointer, static::MAX_ORG_RECORD_LENGTH);
        } else {
            fseek($this->filehandle, $recordPointer, SEEK_SET);
            $orgBuf = fread($this->filehandle, static::MAX_ORG_RECORD_LENGTH);
        }
    ...
    /**
     * Replacement for the _get_region
     *
     * @param $ipNum
     * @return array
     */
    protected function _getRegion($ipNum)
    {
        if ($this->databaseType == static::GEOIP_REGION_EDITION_REV0) {
            $seekRegion = $this->_geoipSeekCountry($ipNum) - static::GEOIP_STATE_BEGIN_REV0;
            if ($seekRegion >= 1000) {
                $countryCode = "US";
                $region = chr(($seekRegion - 1000) / 26 + 65) . chr(($seekRegion - 1000) % 26 + 65);
            } else {
                $countryCode = $this->geoipCountryCodes[$seekRegion];
                $region = "";
            }

            return array($countryCode, $region);
        } elseif ($this->databaseType == static::GEOIP_REGION_EDITION_REV1) {
            $seekRegion = $this->_geoipSeekCountry($ipNum) - static::GEOIP_STATE_BEGIN_REV1;
            if ($seekRegion < static::US_OFFSET) {
                $countryCode = "";
                $region = "";
            } elseif ($seekRegion < static::CANADA_OFFSET) {
                $countryCode = "US";
                $region = chr(($seekRegion - static::US_OFFSET) / 26 + 65) . chr(($seekRegion - static::US_OFFSET) % 26 + 65);
            } elseif ($seekRegion < static::WORLD_OFFSET) {
                $countryCode = "CA";
                $region = chr(($seekRegion - static::CANADA_OFFSET) / 26 + 65) . chr(($seekRegion - static::CANADA_OFFSET) % 26 + 65);
            } else {
                $countryCode = $this->geoipCountryCodes[($seekRegion - static::WORLD_OFFSET) / static::FIPS_RANGE];
                $region = "";
            }
    ...
    /**
     * Replace for the geoip_close function
     */
